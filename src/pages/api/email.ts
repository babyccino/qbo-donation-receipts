...
import { EmailHistoryItem, FileStorage, UserData } from "@/types/db"
import { Donation } from "@/types/qbo-api"
import { user as firebaseUser, fileStorage as firebaseStorage } from "@/lib/db"
import { isUserDataComplete } from "@/lib/db/db-helper"
...
export const createHandler = (user: UserData, fileStorage: FileStorage) =>
  (async (req, res, session) => {
    ...
    const [donations, signatureWebpDataUrl, logoWebpDataUrl] = await Promise.all([
      ...
      fileStorage.downloadFileBase64DataUrl(userData.id, donee.signature),
      fileStorage.downloadFileBase64DataUrl(userData.id, donee.smallLogo),
    ])
    ...
    await user.append(session.user.id, {
      emailHistory: [emailHistoryItem],
    })
    res.status(200).json({ ok: true })
  }) satisfies AuthorisedHandler

export default createAuthorisedHandler(authOptions, createHandler(firebaseUser, firebaseStorage), [
  "POST",
])
