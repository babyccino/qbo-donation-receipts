...
const handler: AuthorisedHandler = async ({ body, method }, session) => {
  ...
  if (method === "GET") return NextResponse.redirect(stripeSession.url)
  else return NextResponse.json({ url: stripeSession.url }, { status: 200 })
}
