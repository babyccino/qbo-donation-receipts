...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  ...
  return {
    ...token,
    accessToken: (refreshedTokens as any).access_token,
    accessTokenExpires: Date.now() + MS_IN_HOUR,
    refreshToken: (refreshedTokens as any).refresh_token ?? token.refreshToken,
  }
}

type QboCallbacksOptions = CallbacksOptions<QBOProfile, QboAccount>
const createSignIn = (userData: UserData) =>
  (async ({ user, account, profile }) => {
    ...
    const [userInfo, companyInfo, dbUserData] = await Promise.all([
    ...
    await userData.set(id, set)
    return true
  }) satisfies QboCallbacksOptions["signIn"]
...
export const createAuthOptions = (user: UserData) =>
  ({
    pages: {
      signIn: "/auth/signin",
    },
